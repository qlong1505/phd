...
n=1;
formatOut = 'yyyymmddHHMMSS';
dateTime = datestr(now,formatOut);
% for file_list=[...
...
for file_list=[...
    ...
    fileID = fopen(strcat(file(1:(end-18)),dateTime,'.csv'),'w');
    ...
    toc
    %csvwrite(strcat(file(1:(end-4)),'.csv'),overshoots);
    dlmwrite(strcat(file(1:(end-18)),dateTime,'.csv'),[RiseTime',SettlingMax'...
...
%     if strfind(file_list,"bruteforce")==1
%         histogram(SettlingTime,200,'Normalization','probability');
%     else
        histogram(SettlingTime,100,'Normalization','probability');
%     end
    
%     title('Histogram of original image', 'FontSize', fontSize);
    xlabel('Time (s)', 'FontSize', 18);
    ylabel('Probability', 'FontSize', 18);
    xlim([11 14.5]); % Scale x axis manually.
    ...
    set(gca, 'FontSize', 20)
    title(strcat('SettlingTime_',file(1:(end-19))),'Interpreter','none');
    n= n+1;
    subplot(3,2,n);
    
    
%     if strfind(file_list,"bruteforce")==1
%         histogram(Peak,200,'Normalization','probability');
%     else
%         histogram(Peak,200,'Normalization','probability');
%     end
%     histogram(Peak,200);%,'Normalization','probability');
histfit(Peak,200);
    title(strcat('Peak_',file(1:(end-19))),'Interpreter','none');
%     xlabel('Amplitude', 'FontSize', 18);
%     ylabel('Probability', 'FontSize', 18);
%     xlim([1.3 1.53]); % Scale x axis manually.
%     ylim([0 0.018]); % Scale x axis manually.
    xlim([1.45 1.5]); % Scale x axis manually.
     if strfind(file_list,"bruteforce")>=1
        ylim([0 300])
    else
        ylim([0 50])
    ...
    FITDIST = fitdist(Peak','normal')
    [h,p,st]= chi2gof(Peak','cdf',{@normcdf,FITDIST.mu,FITDIST.sigma})
...
%% use fitdist 20190703
pd = makedist('Normal');
rng default;  % for reproducibility
x = random(pd,100,1);
h = chi2gof(x,'Alpha',0.05)